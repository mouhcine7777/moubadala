...
function InfoChip({ icon, value }: { icon: React.ReactNode; value: string | null }) {
  ...
  return (
    <div className="flex items-center gap-1.5 text-xs text-[#0D3B66]/70 bg-[#EEF3F8] px-3 py-1.5 rounded-full">
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5 flex flex-col gap-3">
      <div className="flex items-center justify-between">
        <p className="text-sm font-bold text-[#0D3B66]">Compléter votre profil</p>
        <span className={clsx(
      ...
      {missing.length > 0 && (
        <div className="flex flex-col gap-1.5">
          <p className="text-xs text-gray-400">Champs manquants :</p>
          ...
          <Link
            ...
            className="text-xs text-[#0D3B66] font-semibold underline underline-offset-2 mt-1"
          >
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-6 py-8 flex flex-col gap-8">
        ...
        {profile.status === 'rejected' && (
          <div className="bg-red-50 border border-red-200 rounded-xl px-5 py-4 flex items-center gap-3">
            ...
            <div className="flex-1">
              <p className="text-sm font-semibold text-red-800">Profil rejeté</p>
            ...
            <Link
              href="/dashboard/profil"
              className="text-xs font-semibold text-red-700 border border-red-300 hover:border-red-400 px-3 py-1.5 rounded-lg transition-colors shrink-0"
            >
              Modifier →
            </Link>
          </div>
        ...
        {profileComplete ? (
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
            <div className="bg-gradient-to-r from-[#0D3B66] to-[#1a5a9a] px-6 py-5 flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-white/60 text-xs">{profile.sector}</p>
                </div>
              </div>
              <Link
                href="/dashboard/profil"
            ...
            <div className="px-6 py-4 flex flex-wrap gap-2 border-b border-gray-100">
              <InfoChip icon={<MapPin size={11}/>}  value={[profile.city, profile.prefecture].filter(Boolean).join(', ')}/>
              <InfoChip icon={<Mail size={11}/>}    value={profile.email}/>
              <InfoChip icon={<Phone size={11}/>}   value={profile.phone}/>
              <InfoChip icon={<User size={11}/>}    value={profile.contact_name}/>
              <InfoChip icon={<MapPin size={11}/>}  value={profile.address}/>
            </div>
            ...
            {profile.description && (
              <div className="px-6 py-4 border-t border-gray-100">
                <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-1.5">Activité</p>
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-xl px-5 py-4 flex items-center justify-between gap-4">
            <p className="text-sm text-amber-800">Complétez votre profil entreprise pour publier des annonces.</p>
            <Link href="/dashboard/profil" className="text-sm font-semibold text-amber-800 underline underline-offset-2 shrink-0">
              Compléter →
            </Link>
          </div>
        )}
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { label: 'Total annonces', value: stats.total,     icon: <FileText size={18}/>,    color: 'text-[#0D3B66]' },
            { label: 'Publiées',       value: stats.published, icon: <CheckCircle size={18}/>, color: 'text-green-600' },
            { label: 'En attente',     value: stats.pending,   icon: <Clock size={18}/>,       color: 'text-amber-600' },
            { label: 'En pause',       value: stats.paused,    icon: <PauseCircle size={18}/>, color: 'text-gray-400'  },
          ].map(stat => (
            <div key={stat.label} className="bg-white rounded-xl border border-gray-100 p-5 flex flex-col gap-2 shadow-sm">
              <div className={clsx('opacity-80', stat.color)}>{stat.icon}</div>
              <p className="text-2xl font-bold text-[#0D3B66]">{stat.value}</p>
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-base font-bold text-[#0D3B66]">Mes annonces</h2>
            <Link href="/dashboard/annonces" className="text-xs text-[#0D3B66] hover:underline">
              Gérer tout →
            </Link>
          </div>
          {listings.length === 0 ? (
              ...
              {listings.map(listing => (
                ...
                >
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <span className={clsx('text-xs font-semibold px-3 py-1 rounded-full', STATUS_COLOR[listing.status])}>
                      {STATUS_LABEL[listing.status]}
                    </span>
                    <Link
                      href={`/dashboard/annonces/${listing.id}`}
                      className="flex items-center gap-1.5 text-xs text-gray-400 hover:text-[#0D3B66] border border-gray-200 hover:border-gray-300 px-3 py-1.5 rounded-lg transition-colors"
                    >
                      <Pencil size={12}/> Modifier
                    </Link>
                  </div>
        ...
        {similarListings && similarListings.length > 0 && (
          <div>
            <div className="flex items-center justify-between mb-4">
              ...
              <Link href="/moubaplace" className="text-xs text-[#0D3B66] hover:underline">
                Voir tout →
              </Link>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {similarListings.map((listing: any) => (
                <Link
                  ...
                  className="bg-white rounded-xl border border-gray-100 p-4 hover:shadow-md transition-shadow flex flex-col gap-2"
                >
                  <div className="flex items-center justify-between">
                    <span className={clsx(
                  ...
                  <p className="text-sm font-semibold text-[#0D3B66] line-clamp-2 leading-snug">
                    {listing.title}
                  </p>
                  ...
                  {listing.profiles?.company_name && (
                    <p className="text-xs text-gray-400">{listing.profiles.company_name} · {listing.city}</p>
                  )}
                </Link>
              ))}
            </div>
