    ...
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
      ...
      <div className="px-5 py-4 flex items-start justify-between gap-4">
        <div className="flex flex-col gap-2 flex-1 min-w-0">
          ...
          {listing && (
            <div className="flex items-center gap-2">
              <p className="text-xs text-gray-400">Annonce :</p>
              <Link
        ...
        {partner && (
          <div className="shrink-0 flex flex-col items-end gap-1 text-right">
            ...
            <p className="text-xs font-semibold text-[#0D3B66]">{partner.company_name}</p>
      ...
      {(request.status === 'accepted' || request.status === 'finalizing') && (
        <div className="px-5 py-3 bg-green-50 border-t border-green-100 flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-4 flex-wrap">
            <p className="text-xs font-semibold text-green-700">Coordonnées :</p>
            {partner?.email && (
              <a href={`mailto:${partner.email}`} className="text-xs text-green-700 hover:underline">
                {partner.email}
              </a>
            )}
            {partner?.phone && (
              <a href={`tel:${partner.phone}`} className="text-xs text-green-700 hover:underline">
                {partner.phone}
              </a>
            )}
          </div>
          <Link
            ...
            className="flex items-center gap-1.5 text-xs font-semibold text-white bg-[#0D3B66] hover:bg-[#0a2f52] px-3 py-2 rounded-lg transition-colors"
          >
      ...
      {mode === 'received' && request.status === 'pending' && (
        <div className="px-5 py-3 border-t border-gray-100 flex items-center gap-2 flex-wrap">
          ...
          >
            <CheckCircle size={13}/>{loading ? '...' : 'Accepter'}
          </button>
          <button
            disabled={loading}
            onClick={() => handleStatus('finalizing')}
            className="flex items-center gap-1.5 text-xs font-semibold text-purple-700 bg-purple-50 hover:bg-purple-100 border border-purple-200 px-3 py-2 rounded-lg transition-colors disabled:opacity-40"
          >
            <Handshake size={13}/>{loading ? '...' : 'En finalisation'}
          </button>
          <button
            disabled={loading}
            onClick={() => handleStatus('refused')}
            className="flex items-center gap-1.5 text-xs font-semibold text-red-500 bg-red-50 hover:bg-red-100 border border-red-100 px-3 py-2 rounded-lg transition-colors disabled:opacity-40"
          >
            <XCircle size={13}/>{loading ? '...' : 'Refuser'}
          </button>
        </div>
      )}
      ...
      {request.status === 'finalizing' && mode === 'received' && (
        <div className="px-5 py-3 border-t border-purple-100 bg-purple-50 flex items-center justify-between gap-3 flex-wrap">
          ...
          <Link
            ...
            className="flex items-center gap-1.5 text-xs font-semibold text-white bg-purple-600 hover:bg-purple-700 px-3 py-2 rounded-lg transition-colors shrink-0"
          >
            <FileText size={13}/> Créer transaction
          </Link>
        </div>
      )}
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="flex gap-2">
        ...
        >
          ...
          {sent.length > 0 && (
            <span className={clsx(
              'ml-1.5 text-[10px] font-bold px-1.5 py-0.5 rounded-full',
              tab === 'sent' ? 'bg-white/20 text-white' : 'bg-gray-100 text-gray-500'
            )}>
              {sent.length}
            </span>
          )}
        ...
        <div className="flex flex-col gap-3">
          {received.length === 0 ? (
            <div className="bg-white rounded-xl border border-gray-100 p-12 text-center shadow-sm">
              ...
              <p className="text-gray-400 text-sm">Aucune demande reçue pour le moment.</p>
            </div>
          ) : (
            received.map(r => (
              <RequestCard
      ...
      {tab === 'sent' && (
        <div className="flex flex-col gap-3">
          {sent.length === 0 ? (
            <div className="bg-white rounded-xl border border-gray-100 p-12 text-center shadow-sm">
              <Handshake size={28} className="text-gray-200 mx-auto mb-3"/>
              <p className="text-gray-400 text-sm">Vous n'avez pas encore envoyé de demande.</p>
              <Link
                href="/moubaplace"
                className="inline-block mt-3 text-sm text-[#0D3B66] font-semibold underline underline-offset-2"
              >
